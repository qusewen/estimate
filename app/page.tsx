        ...
        <div className="flex-1">
          <div className="relative">
            <Input
                ...
                placeholder="0"
                value={d}
                onChange={handleNumberInput(setD)}
                className="pr-8"
            />
        ...
        <div className="flex-1">
          <div className="relative">
            <Input
                ...
                placeholder="0"
                value={h}
                onChange={handleNumberInput(setH)}
                className="pr-8"
            />
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground pointer-events-none">
            ч
        ...
        <div className="flex-1">
          <div className="relative">
            <Input
                ...
                placeholder="0"
                value={m}
                onChange={handleNumberInput(setM)}
                className="pr-8"
            />
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground pointer-events-none">
            м
...
export default function TaskEstimator() {
  ...
  const calcTotalHours = useCallback((d: string, h: string, m: string) => {
  ...
  const formatEffort = useCallback((d: string, h: string, m: string) => {
    const dNum = parseInt(d) || 0
    const hNum = parseInt(h) || 0
    const mNum = parseInt(m) || 0
  ...
  const calcTestHours = useCallback(() => {
  ...
  }, [testPercent, days, hours, minutes, testDays, testHours, testMinutes, calcTotalHours])

  const formatTestEffort = useCallback(() => {
    const percent = parseInt(testPercent) || 0
    if (percent > 0) {
  ...
  const formatTotalSum = useCallback(() => {
    ...
    return `${parts.join(" ")}`
  }, [days, hours, minutes, reviewDays, reviewHours, reviewMinutes, bufferDays, bufferHours, bufferMinutes, calcTotalHours, calcTestHours])

  const formatPerformerLevel = useCallback(() => {
  ...
  }, [days, hours, minutes, reviewDays, reviewHours, reviewMinutes, bufferDays, bufferHours, bufferMinutes, testPercent, assumptions, dependencies, performerLevel, estimatedBy, formatEffort, formatTestEffort, formatTotalSum, formatPerformerLevel, formatEstimatedBy])
